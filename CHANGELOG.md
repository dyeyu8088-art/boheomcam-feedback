...
## [0.3.4] - 2026-09-04 手机横屏走查修复（PHASE 24 · P11）

- 捕鱼 `FishRig` 支持方案 A 帧序列（`frames: {key, cols, rows, count, fps}`，sheet 存在即逐帧播放，缺失退回网格骨骼）；docs/13 §10 记录接入方式与素材搜索结果
- 麻将桌手机横屏（高 ≤ 450）：四家牌河缩小成环（8 列 × 0.62）、中央罗盘缩小并置于牌河之上、对家座位牌上移、托管条移到手牌上方 —— 不再压住罗盘 / 手牌 / 对家暗牌
- 登录页响应式：短屏缩小徽记 / 字标并允许滚动；手机横屏改为两栏网格（品牌 | 登录卡 / 语言 / 协议），徽记不再被裁切
- 默认昵称 / 陪练名中韩双语：`@yanbian/game-common/names`（服务端落库中文单一来源，客户端 `displayName()` 按语言显示；大厅 / 牌桌 / 捕鱼 / 排行 / 好友 / 战绩 / 赛事全部接入）
- `tests/phone-play.mjs` 弹层关闭选择器修复（`.gp-close, .ms-close, button.x`）

## [0.3.3] - 2026-09-04 捕鱼动画系统 v2 · 全项目静态图检查（PHASE 23 · P14）

### 捕鱼（packages/game-common + apps/client-game）
- 共享路径库重写：17 条路径（直线 / 斜向 / 弧 / 深 S / 停留变向 / 之字 / Boss 环绕 / Boss 慢 S）+ 速度曲线（拐点减速、停留）+ `headingOnPath` + 鱼群车道 `laneForFish`；服务端判定与客户端渲染同函数（4 个新单测）
- `fishRig.ts`：MeshPlane 网格骨骼（脊柱正弦波逐节传递 / 圆身呼吸 / 海龟鳍脚 / 人形袍摆）+ 叠加层（胸鳍扇动、眨眼、张嘴，配色采样自素材）+ 状态机 spawn / swim / turn / hit / stun / escape / death；局部动作 9–14 FPS 节流、随机相位、游速联动
- `ocean.ts`：远 / 中 / 近三层视差环境（光束摆动、焦散、海床、水草、颗粒、气泡池、鱼尾迹气泡）
- FishingView：受击闪白 + 光环 + 位移，捕获翻肚下沉 + 金币从鱼位爆出（大鱼 / Boss 更强 + 震动 + 气泡），Boss 警告震动 / 愤怒（HP<40%）/ 死亡；设备分级、页面隐藏暂停、减少动态；DEV 下 `__fs.stats` 每帧更新耗时
- 验收脚本 `tests/fishing-verify.mjs`：录像、重新居中帧差（局部动作证据）、状态 / 鱼种 / 路径观察、`ONLY=turn` 转向观察、桌面与手机（CPU 4×）每帧更新耗时；文档 `docs/13-fishing-animation.md`（含素材类型 / 帧数 / 许可证 / 临时标记表）

### 其它游戏静态图修复
- 麻将吃 / 碰 / 杠喊话加冲击波环 + 火花；胡牌双层冲击波 + 火花
- 红十结算 WIN / ×2 / ×4 弹入
- 水果机 / 轮盘 / 股票吉祥物：浮动 + 呼吸 + 微摆、结果面板弹入（整图动画，素材待拆分）
